var DragRace = function(lobby, players){
	...
	this.dragonWidth = 350;
	this.dragonHeight = 225;
	...
	this.start = function(){
		...
		this.track.length = 18000;
		this.track.obstacles = [
			{type: "hurdle", x: 2000, y: 600},
			{type: "hurdle", x: 3000, y: 600},
			{type: "hurdle", x: 3500, y: 600},
			{type: "hurdle", x: 4800, y: 600},
			{type: "flyingHurdle", x: 7000, y: 420},
			{type: "flyingHurdle", x: 7500, y: 320},
			{type: "ring", x: 8000, y: 350},
			{type: "flyingHurdle", x: 8500, y: 380},
			{type: "ring", x: 10000, y: 200},
			
			{type: "flyingHurdle", x: 13000, y: 300},
			{type: "hurdle", x: 12800, y: 600},
			{type: "hurdle", x: 14500, y: 600},
			{type: "hurdle", x: 16000, y: 600},
			{type: "ring", x: 15950, y: 400}
		];
		
		this.track.markers = {
			6000: "fly", // Markers should be placed at multiples of the track sprite width.
			12000: "run"
		}
			...
			switch(dragon.raceState){
				case "Racing":
					...
					if(dragon.boosting){
						dragon.vx += 0.40;
						dragon.boostTimer --;
						if(dragon.boostTimer == 0){
							dragon.boosting = false;
						}
					}else{
						dragon.vx = Math.min(10, dragon.vx + 0.08);
					}
					this.checkObstacles();
					break;
				...
				switch(obst.type){
					...
					case "hurdle":
						if(obst.triggered.indexOf(id) == -1){
							...
							var xCollision = dragon.x > obst.x - hurdleWidth/2 - (this.dragonWidth * 0.50)/2 && dragon.x < obst.x + hurdleWidth/2 + (this.dragonWidth*0.30)/2;
							var yCollision = dragon.y > obst.y - hurdleHeight + (this.dragonHeight * 0.2) && dragon.y < obst.y + (this.dragonHeight*0.75);
					...
					case "flyingHurdle":
						if(obst.triggered.indexOf(id) == -1){
							...
							var xCollision = dragon.x > obst.x - hurdleWidth/2 - (this.dragonWidth * 0.50)/2 && dragon.x < obst.x + hurdleWidth/2 + (this.dragonWidth*0.30)/2;
							var yCollision = dragon.y > obst.y - hurdleHeight + (this.dragonHeight * 0.20) && dragon.y < obst.y + (this.dragonHeight*0.75);
							if(xCollision && yCollision){
								obst.triggered.push(id);
					...
					case "runMarker":
						...
						break;
					case "ring":
						if(obst.triggered.indexOf(id) == -1 && dragon.x >= obst.x - 25 && Math.abs(dragon.y - this.dragonHeight/2 - obst.y) <= 125){
							obst.triggered.push(id);
							dragon.boosting = true;
							dragon.boostTimer = 30;
						}
						break;
				}
			}
	...
	*/
	this.getStateForClient = function(socketID){
		var gameState = {};
		var otherDragons = [];
		var clientDragon = {x: this.dragons[socketID].x, y: this.dragons[socketID].y, vx: this.dragons[socketID].vx, vy:this.dragons[socketID].vy};
		for(var otherPlayerID in this.players){
			if(otherPlayerID != socketID){
				otherDragons.push({
					...
					vx: this.dragons[otherPlayerID].vx,
					vy: this.dragons[otherPlayerID].vy
				});
			}
		}
