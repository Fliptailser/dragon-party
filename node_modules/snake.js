var Snake = function(lobby, players){
	...
	this.start = function(){
		...
		for(var playerSocketID in this.players){
			...
			this.snakes[playerSocketID] = {name: this.players[playerSocketID].name, x: spawnX, y: spawnY, tail: [position - this.width, position - 1 - this.width, position - 1], dir: 0, growth: 0, alive: true};
			this.liveSnakes += 1;
		...
		switch(this.state){
			...
			case "running":
				if(this.timer == this.speed){
					...
					// Move imaginary BGM playhead forward (gets sent to clients)
	...
	*/
	this.getStateForClient = function(socketID){
		...
		gameState.playhead = this.playhead;
